...
const POSITION_DESCRIPTIONS: Record<PositionValue, string> = {
  static:   'Default flow. Offset properties (top/left/right/bottom) have no effect.',
  relative: 'Stays in normal flow but can be nudged with offsets. Space is preserved.',
  ...
  sticky:   'Hybrid: acts as relative until it hits a scroll threshold, then sticks.',
}

const POSITION_PRESETS: { label: string; position: PositionValue; offsets: Partial<Offsets>; transform?: string }[] = [
  { label: 'Top-left corner',  position: 'absolute', offsets: { top: '0',   left: '0',   right: '',    bottom: '' } },
  { label: 'Top-right corner', position: 'absolute', offsets: { top: '0',   left: '',    right: '0',   bottom: '' } },
  { label: 'Bottom-left',      position: 'absolute', offsets: { top: '',    left: '0',   right: '',    bottom: '0' } },
  { label: 'Bottom-right',     position: 'absolute', offsets: { top: '',    left: '',    right: '0',   bottom: '0' } },
  { label: 'Centered badge',   position: 'absolute', offsets: { top: '50%', left: '50%', right: '',    bottom: '' }, transform: 'translate(-50%, -50%)' },
]

function OffsetInput({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
...
export function PositionPage() {
  ...
  const [offsets, setOffsets] = useState<Offsets>({ top: '', left: '', right: '', bottom: '' })
    ...
    <div className="flex flex-1 overflow-hidden min-h-0">
      ...
      <div className="w-60 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 overflow-y-auto p-3 shrink-0">
        ...
        <Select
          label="position"
          value={position}
          options={(['static', 'relative', 'absolute', 'fixed', 'sticky'] as PositionValue[]).map((v) => ({ value: v, label: v }))}
          onChange={(v) => { setPosition(v as PositionValue); setLiveTransform('') }}
        />

        <SectionTitle>Offsets</SectionTitle>
        <div className={clsx('grid grid-cols-2 gap-2 mb-3', isStatic && 'opacity-40 pointer-events-none')}>
          <OffsetInput label="top"    value={offsets.top}    onChange={(v) => setOffset('top', v)} />
          <OffsetInput label="right"  value={offsets.right}  onChange={(v) => setOffset('right', v)} />
          <OffsetInput label="bottom" value={offsets.bottom} onChange={(v) => setOffset('bottom', v)} />
          <OffsetInput label="left"   value={offsets.left}   onChange={(v) => setOffset('left', v)} />
        </div>
        {isStatic && (
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 bg-slate-50 dark:bg-slate-950 overflow-auto">
        ...
        <div className="w-full max-w-[700px] px-3 py-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[11px] font-mono text-slate-500 dark:text-slate-400 leading-relaxed text-center">
          <span className="text-brand-600 dark:text-brand-400 font-bold">{position}</span>
          {' — '}
        ...
        <div className="grid grid-cols-5 gap-3 w-full max-w-[700px]">
          {(['static', 'relative', 'absolute', 'fixed', 'sticky'] as PositionValue[]).map((p) => (
            <button
              key={p}
              onClick={() => { setPosition(p); setLiveTransform('') }}
              className={clsx(
            ...
            >
              <div className="w-full h-12 rounded relative bg-slate-100 dark:bg-slate-800 overflow-hidden">
                ...
                <div
                  ...
                  style={{
                    ...
                    top:    p === 'relative' ? 4  : p === 'absolute' ? 2  : p === 'fixed' ? 0 : p === 'sticky' ? 2 : undefined,
                    right:  p === 'fixed' ? 1 : undefined,
                    left:   p === 'absolute' ? 10 : p === 'relative' ? 6 : p === 'sticky' ? 6 : undefined,
              ...
              <span className={clsx(
                ...
                position === p ? 'text-brand-600 dark:text-brand-400' : 'text-slate-500 dark:text-slate-400'
              )}>
                {p}
              </span>
            </button>
          ...
          {!isStatic && (
            <div className="absolute bottom-2 right-3 flex gap-1.5 flex-wrap justify-end">
              {(['top', 'left', 'right', 'bottom'] as const).map((k) =>
                offsets[k].trim() ? (
                  <span key={k} className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[9px] font-mono text-slate-500 dark:text-slate-400">
                    {k}: {offsets[k]}
                  </span>
                ) : null
              )}
