  ...
  return (
    <div className="flex-1 overflow-auto p-4 md:p-6">
      <div className="max-w-3xl mx-auto flex flex-col gap-5 md:gap-6">
        <div>
          <h1 className="font-mono font-extrabold text-lg md:text-xl text-slate-900 dark:text-slate-100">
            {t("reverseMode")}
          </h1>
          <p className="text-[12px] font-mono text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
            Paste any CSS snippet — the tool parses it and applies the relevant
            controls to the Flexbox playground automatically.
          </p>
        </div>

        <div className="flex gap-3 p-3 md:p-4 rounded-xl bg-brand-50 dark:bg-brand-950/20 border border-brand-200 dark:border-brand-900 text-[12px] font-mono text-brand-700 dark:text-brand-300">
          <Zap size={16} className="shrink-0 mt-0.5" />
          <span className="leading-relaxed">{t("reverseHint")}</span>
        </div>
        ...
        {parsed && Object.keys(parsed).length === 0 && (
          <div className="p-3 md:p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 text-[12px] font-mono text-amber-700 dark:text-amber-300 leading-relaxed">
            No recognizable CSS layout properties found. Try adding flex, grid,
            or position rules.
          </div>
        )}
      </div>
