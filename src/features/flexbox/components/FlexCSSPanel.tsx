...
interface FlexCSSPanelProps {
  mobile?: boolean;
}

export function FlexCSSPanel({ mobile = false }: FlexCSSPanelProps) {
        ...
        {tab === "css" ? (
          ...
          <>
            <div className="rounded-lg bg-slate-950 border border-slate-800 p-3">
              <p className="font-mono text-[11px] text-cyan-300 leading-relaxed break-all">
                {tw}
              </p>
            </div>
            <Button
          ...
          <div className="rounded-lg bg-brand-50 dark:bg-brand-950/20 border border-brand-200 dark:border-brand-900 p-2 flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[10px] text-brand-700 dark:text-brand-300 font-semibold">
                Main:{" "}
                {store.direction.includes("column") ? "vertical" : "horizontal"}
              </span>
            </div>
            <div className="flex items-center gap-2">
