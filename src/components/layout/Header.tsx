...
export function Header({ onShowExport, onShowKeyboard, currentPage }: HeaderProps) {
  ...
  return (
    <header className="h-13 flex items-center justify-between px-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shrink-0 z-50">
      {/* Left */}
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-extrabold text-[14px] text-slate-900 dark:text-slate-100">
            CSS Layout Lab
          </span>
        </div>
        ...
        <span className="font-mono font-semibold text-[11px] uppercase tracking-wider text-slate-400 dark:text-slate-500">
      ...
      {/* Right */}
      <div className="flex items-center gap-2">
        <Button size="sm" variant="ghost" onClick={onShowKeyboard}>
          <Keyboard size={13} />
        </Button>

        <Button size="sm" onClick={onShowExport}>
          <Download size={12} />
          {t('export')}
        </Button>
